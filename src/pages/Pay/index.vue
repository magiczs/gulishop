    ...
    async pay() {
      ...
      try {
        ...
        this.$alert(`<img src="${imgUrl}"/>`, "请使用微信扫码支付", {
          ...
          //关闭之前回调,如果不写这个回调,那么无论点击什么按钮,消息盒子都会强制关闭,如果写了这个回调,那么消息盒子的关闭由我们自己控制
          beforeClose: (action, instance, done) => {
            if (action === "confirm") {
              // if (this.status !== 200) {
              //   this.$message.warning("支付未成功,支付后自动跳转");
              // }
          ...
          .then(() => {})
          .catch(() => {}); //函数的返回值也是promise

        //3\弹出消息同时循环的给后台发请求，获取该订单的支付状态数据
        //弹出消息同时循环的给后台发请求,获取该订单的支付状态数据
        ...
        if (!this.timer) {
          this.timer = setInterval(async () => {
            ...
            if (result.code === 200) {
              //支付成功:停止定时器  跳转到支付成功页面 把当前的状态保存起来 以便用户点击我已成功支付的时候去判定
              this.status = 200;
              clearInterval(this.timer); //clearInterval清除定时器,停止给定编号的定时器,并没有清空存储编号的变量
              this.timer = null;
              //成功支付后手动关闭弹窗
